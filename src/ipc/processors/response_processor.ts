      ...
      if (uncommittedFiles.length > 0) {
        ...
        try {
          commitHash = await gitCommit({
            path: appPath,
            message: message + " + extra files edited outside of Core77",
            amend: true,
          });
          logger.log(
            `Amend commit with changes outside of core77: ${uncommittedFiles.join(", ")}`,
          );
        } catch (error) {
          // Just log, but don't throw an error because the user can still
          // commit these changes outside of Core77 if needed.
          logger.error(
            `Failed to commit changes outside of core77: ${uncommittedFiles.join(
              ", ",
            )}`,
          );
