...
export async function createFromTemplate({
  ...
  // If no template is selected or templates array is empty, create an empty app directory
  if (!templateId) {
    logger.info(`No template selected, creating empty app directory at ${fullAppPath}`);
  ...
  if (templateId === "react") {
    ...
    if (fs.existsSync(scaffoldPath) && fs.readdirSync(scaffoldPath).length > 0) {
      await copyDirectoryRecursive(scaffoldPath, fullAppPath);
    } else {
      logger.info(`Scaffold directory is empty, creating empty app directory at ${fullAppPath}`);
      await fs.ensureDir(fullAppPath);
    }
    return;
  }
  ...
  try {
    ...
    logger.warn(`Failed to create from template ${templateId}, creating empty app directory instead:`, error);
    await fs.ensureDir(fullAppPath);
  }
}
