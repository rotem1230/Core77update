...
export function registerChatStreamHandlers() {
  ipcMain.handle("chat:stream", async (event, req: ChatStreamParams) => {
    try {
      // Rate limiting: prevent too many concurrent streams
      const MAX_CONCURRENT_STREAMS = 2;
      if (activeStreams.size >= MAX_CONCURRENT_STREAMS) {
        throw new Error("Too many active requests. Please wait for current requests to complete before starting a new one.");
      }
        ...
        const simpleStreamText = async ({
          ...
          return streamText({
            ...
            onError: (error: any) => {
              ...
              // Provide a better message for overload errors
              let finalMessage;
              if (message.includes("Overloaded")) {
                finalMessage = "Core77 is currently experiencing high demand. Please wait a moment and try again.";
              } else {
                finalMessage = `Sorry, there was an error from Core77: ${message}`;
              }
        ...
        try {
          ...
          if (
            ...
            while (
              ...
              // No delay needed between continuation attempts
        ...
        if (
          ...
          if (status.error) {
            safeSend(
              event.sender,
              "chat:response:error",
              `Sorry, there was an error applying Core77's changes: ${status.error}`,
            );
          }
